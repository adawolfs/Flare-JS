...
After the installation completes, copy `gl-matrix.js` from `node_modules/gl-matrix/dist` into the repo's `/build` folder. _(N.B. `canvaskit` is downloaded with this repo, and already present in the `/canvaskit` folder.)
```
cp node_modules/gl-matrix/dist/gl-matrix.js build/
cp canvaskit/canvaskit.js build
```
